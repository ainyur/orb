...
static bool cast_fonts(const char* dir, orb_assets* assets, orb_error* err) {
    ...
    return orb_cast_game(&scratch, &out, DIR, &manifest, &result, err) &&
...
int main(void) {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        CHECK(!cast_fonts(dir, &assets, &err));
    ...
    orb_font font = ORB_FONT(orb_asset_find(&table, ORB_ASSET_FONT, orb_asset_id("body", "font")));

    // '!' is codepoint 33, cell 1, advance 4; '"' is 34, cell 2, advance 5.
    CHECK_EQ(orb_text_measure(&table.assets, font, "!\"").width, 9);
    CHECK_EQ(orb_text_measure(&table.assets, font, "!\"").height, 6);
    CHECK_EQ(orb_text_measure(&table.assets, font, "").width, 0);
    CHECK_EQ(orb_text_measure(&table.assets, font, "").height, 0);
    CHECK_EQ(orb_text_measure(&table.assets, ORB_NO_FONT, "!").width, 0);
    CHECK_EQ(orb_text_measure(&table.assets, font, nullptr).width, 0);
    // neither byte is drawable
    CHECK_EQ(orb_text_measure(&table.assets, font, "\n\t").width, 0);
    CHECK_EQ(orb_text_measure(&table.assets, font, "\n\t").height, 6);
    ...
    orb_fb_clear(&fb, 0);
    orb_text_draw(&fb, &table.assets, font, "\"", (orb_vec2) {0, 0}, nullptr);
    ...
    orb_fb_clear(&fb, 0);
    orb_text_draw(&fb, &table.assets, font, "\"", (orb_vec2) {-2, -3}, nullptr);
    CHECK(fb.px[0] != 0);

    // Clipped at the bottom-right: no wrap into the next row.
    orb_fb_clear(&fb, 0);
    orb_text_draw(&fb, &table.assets, font, "\"", (orb_vec2) {14, 5}, nullptr);
    CHECK(fb.px[5 * 16 + 15] != 0);
    CHECK_EQ(fb.px[6 * 16], 0);

    // Wholly off the left edge: the blitter clips, the pen still advances.
    orb_fb_clear(&fb, 0);
    orb_text_draw(&fb, &table.assets, font, "\"\"", (orb_vec2) {-5, 0}, nullptr);
    CHECK(fb.px[0] != 0); // the second glyph starts at -5 + 5 = 0

    // Wholly outside: nothing drawn, no crash.
    orb_fb_clear(&fb, 0);
    orb_text_draw(&fb, &table.assets, font, "\"", (orb_vec2) {100, 100}, nullptr);
    CHECK_EQ(fb.px[0], 0);

    orb_text_draw(&fb, &table.assets, ORB_NO_FONT, "\"", (orb_vec2) {0, 0}, nullptr);
    orb_text_draw(&fb, &table.assets, font, nullptr, (orb_vec2) {0, 0}, nullptr);
    CHECK_EQ(fb.px[0], 0);
