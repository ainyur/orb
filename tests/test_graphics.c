...
int main(void) {
    ...
    CHECK_EQ(fb.px[3 * 8 + 7], 7); // index 0 is transparent
    ...
    // sprites and animation over a hand-built asset set: one 4x2 sheet, two 2x2 sprites
    ...
    CHECK_EQ(orb_animation_step(&as, &st).v, 0);
    // a frame counter that outlived a recast which shrank the animation wraps
    // before it indexes the tables
    st.frame = 5;
    st.ticks = 0;
    CHECK_EQ(orb_animation_step(&as, &st).v, 1);
    CHECK_EQ(st.frame, 0); // frame 1 lasts one tick, so it advanced

    // the api binding routes to the runtime's own framebuffer, palette, and assets
    orb_api_init(&a, 8, 4);
    ...
    // generation, so the handles the running code still holds fail closed until
    // the code reloads against the regenerated header
    ...
    orb_api_set_assets(&as);
    api->clear(0);
    api->sprite_draw(NULL, ORB_SPRITE(1), 0, 0, 0, NULL);
    CHECK_EQ(orb_api_framebuffer()->px[0], 3);

    uint64_t recast_sprite_ids[2] = {11, 33}, recast_animation_ids[1] = {55}; // a new half

    as.sprite_ids = recast_sprite_ids;
    as.animation_ids = recast_animation_ids;
    orb_api_set_assets(&as);
    api->clear(0);
    api->sprite_draw(NULL, ORB_SPRITE(1), 0, 0, 0, NULL); // index 1 changed: nothing
    ...
    orb_api_reset_generations(); // the code reloaded: its header matches again
    api->clear(0);
    api->sprite_draw(NULL, ORB_SPRITE(1), 0, 0, 0, NULL);
    CHECK_EQ(orb_api_framebuffer()->px[0], 3);
