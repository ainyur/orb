...
static bool run_open(orb_error* err) {
    orb_size size = {run_info.width, run_info.height};

    orb_api_init(&run_arena, size);
    run_rgb = orb_arena_push_array(&run_arena, uint32_t, (size_t)size.width* size.height);
...
static bool run_cast(int half, orb_error* err) {
    ...
    if (m.size.width != run_info.width || m.size.height != run_info.height ||
        m.asset_headroom != run_assets[0].size)
...
static bool run_boot_sources(const char* game_dir, orb_error* err) {
    ...
    run_dir = game_dir;
    run_info = (orb_info_desc) {.width = (uint16_t)m.size.width, .height = (uint16_t)m.size.height};
