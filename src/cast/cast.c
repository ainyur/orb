...
// FNV-1a over the generated name: the identity a recast compares per index.
static uint64_t cast_id(const char* name) {
    uint64_t h = 0xcbf29ce484222325u;

    for (const unsigned char* c = (const unsigned char*)name; *c; c++)
        h = (h ^ *c) * 0x100000001b3u;

    return h;
}

static const char* cast_name(orb_arena* a, const char* stem, const char* suffix) {
    size_t n = strlen(stem) + 1 + strlen(suffix) + 1;
    char* s = orb_arena_push(a, n, 1);

    snprintf(s, n, "%s_%s", stem, suffix);

    for (char* c = s; *c; c++)
        *c = isalnum((unsigned char)*c) ? (char)toupper((unsigned char)*c) : '_';
...
static bool cast_body(
    ...
    r->sprite_names = orb_arena_push(scratch, sizeof(char*) * max_sprites, alignof(char*));
    r->animation_names = orb_arena_push(scratch, sizeof(char*) * max_anims, alignof(char*));
    ...
    for (int i = 0; i < m->sprite_count; i++) {
        ...
        for (uint32_t f = 0; f < ase->frame_count; f++) {
            ...
            r->sprite_names[r->sprite_count] = cast_name(scratch, stem, suffix);
            sprite_ids[r->sprite_count] = cast_id(r->sprite_names[r->sprite_count]);
            r->sprite_count++;
        }
        ...
        for (int t = 0; t < ase->tag_count; t++) {
            ...
            r->animation_names[r->animation_count] = cast_name(scratch, stem, tag->name);
            animation_ids[r->animation_count] = cast_id(r->animation_names[r->animation_count]);
            r->animation_count++;
        }
    }
...
static bool cast_write_header(orb_arena* scratch, const orb_cast_result* r, const char* path) {
    size_t cap = 256 + (r->sprite_count + r->animation_count) * 96;
    char* text = orb_arena_push(scratch, cap, 1);
    size_t n = (size_t)snprintf(
        text, cap, "// generated by orb; do not edit\n#pragma once\n#include \"orb.h\"\n\n"
    );

    for (uint32_t i = 0; i < r->sprite_count; i++) {
        n += (size_t)snprintf(
            text + n, cap - n, "#define ORB_SPRITE_%s ORB_SPRITE(%u)\n", r->sprite_names[i], i
        );
    }

    for (uint32_t i = 0; i < r->animation_count; i++) {
        n += (size_t)snprintf(
            text + n, cap - n, "#define ORB_ANIMATION_%s ORB_ANIMATION(%u)\n",
            r->animation_names[i], i
        );
    }

    orb_span existing;

    if (orb_os_read_file(path, scratch, &existing) && existing.len == n &&
        memcmp(existing.ptr, text, n) == 0) {
        return true; // unchanged: leave the mtime alone so scry does not rebuild
    }

    return orb_os_write_file(path, (orb_span) {(uint8_t*)text, n});
}
...
// The header is part of the cast: the game includes it, so a cast whose header
// could not be written is a cast that did not land.
static bool
cast_header(orb_arena* scratch, const char* game_dir, const orb_cast_result* r, orb_error* err) {
    const char* path = cast_path(scratch, game_dir, "game_assets.h");

    if (cast_write_header(scratch, r, path)) return true;

    orb_error_set(err, "cannot write %s", path);
    return false;
}
...
bool orb_cast_game(
    ...
    if (setjmp(recover) == 0) {
        ok = orb_manifest_load(scratch, game_dir, m, err) &&
             cast_body(scratch, out, game_dir, m, r, err) && cast_header(scratch, game_dir, r, err);
    } else {
